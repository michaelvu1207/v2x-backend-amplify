<script lang="ts">
	...
	async function loadSnapshot() {
		if (!config || !hasPerceptionSource(config)) return;
		error = null;
		isLoading = snapshot === null;

		try {
			snapshot = await fetchLivePerceptionDetections();
	...
	<div class="sticky top-0 flex flex-col gap-4 px-4 py-4 lg:w-80">
		<div class="flex items-start justify-between gap-3">
			<div>
				...
				{#if lastUpdated}
					<p class="mt-1 text-[11px] text-gray-500">Updated {lastUpdated}</p>
				{:else}
		...
		<div class="flex flex-col gap-3">
			{#each cameraRows as row}
				{@const detections = row.camera?.detections || []}
				<section class="border border-gray-800 bg-black/70">
					<div class="flex items-center justify-between border-b border-gray-800 px-3 py-2">
						...
						<span class="text-[11px] text-gray-500">{countLabel(row.camera)}</span>
					</div>
