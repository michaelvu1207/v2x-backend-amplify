...
MediaMTX listens only on loopback: RTSP at `127.0.0.1:8554` and low-latency HLS at `127.0.0.1:8888`. nginx exposes HLS under `/camera/`. WebRTC, RTMP, SRT, MoQ, and the MediaMTX API are disabled. The adapter and ffmpeg copy the encoded H.264 stream without transcoding.
...
Recordings are fragmented MP4 under `/var/lib/v2x-camera/recordings/<path>/`. Segments close every 15 minutes and MediaMTX deletes them after seven days (`168h`). To change retention, edit `recordDeleteAfter` in `mediamtx.yml`, rerun `install.sh`, and restart `mediamtx.service` and the relay units.
