...
# enforced in DOCKER-USER) and the host drive, Studio, twin, and loopback-only
# camera relay ports (5199, 8765, 8190, 8865, 8554, 8888, enforced in INPUT).
...
if [ -n "$EXT4" ]; then
    ...
    iptables -C INPUT -i "$EXT4" -p tcp -m multiport --dports 5199,8190,8554,8865,8888 -m conntrack --ctstate NEW -j DROP 2>/dev/null || \
        iptables -I INPUT -i "$EXT4" -p tcp -m multiport --dports 5199,8190,8554,8865,8888 -m conntrack --ctstate NEW -j DROP
...
if [ -n "$EXT6" ]; then
    ...
    ip6tables -C INPUT -i "$EXT6" -p tcp -m multiport --dports 5199,8190,8554,8865,8888 -m conntrack --ctstate NEW -j DROP 2>/dev/null || \
        ip6tables -I INPUT -i "$EXT6" -p tcp -m multiport --dports 5199,8190,8554,8865,8888 -m conntrack --ctstate NEW -j DROP
